<template>
    <CardBoxModal v-model="isActive" title="Quản lý hình ảnh sản phẩm" button="info" has-cancel @confirm="handleSubmit"
        @cancel="handleClose">
        <div class="space-y-6">
            ...
            <div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div v-for="(image, index) in currentImages" :key="index" 
                        ...
                        <img :src="getImageUrl(image)" :alt="'Product image ' + index"
                        ...
                        <div class="absolute inset-0 bg-black bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                            <button @click.prevent="setAsMain(image.id)"
                                ...
                                :class="{'opacity-50 cursor-not-allowed': image.position === 0}"
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-4">
                    ...
                    <div class="w-32 h-32 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center cursor-pointer hover:border-indigo-500"
...
const currentImages = computed(() => {
    return [...(props.product?.media || [])].sort((a, b) => a.position - b.position)
})
...
const handleSubmit = async () => {
    ...
    try {
        ...
        await axios.post(route('products.upload-images', props.product.id), formData, {
            headers: {
...
const getImageUrl = (image) => {
    if (!image || !image.file_path) return '/images/placeholder-product.jpg';
