...
INSERT INTO media (id, type, file_path, mediable_type, mediable_id)
VALUES (1, 'image', '/images/products/cleansing.jpg', 'product', 1),
       (2, 'image', '/images/products/face_wash.jpg', 'product', 2),
       (3, 'image', '/images/products/essence.jpg', 'product', 3),
       (4, 'image', '/images/products/lotion.jpg', 'product', 4),
       (5, 'image', '/images/products/gel.jpg', 'product', 5),
       (6, 'image', '/images/products/sunscreen.jpg', 'product', 6),
       (7, 'image', '/images/products/foundation.jpg', 'product', 7),
       (8, 'image', '/images/users/default.png', 'user', 1);
...
-- Update services
INSERT INTO services (id, category_id, service_name, description, duration, single_price)
VALUES (1, 1, 'Chăm da Amino - Phù hợp mọi loại da', 'Liệu trình chăm sóc da mặt phù hợp cho mọi loại da', 60, 1350000),
       (2, 1, 'Chống lão hoá - Photo', 'Liệu trình chống lão hóa sử dụng công nghệ Photo Facial kết hợp với Tế bào gốc', 45, 1350000),
       (3, 1, 'Dưỡng trắng - Trị nám', 'Liệu trình dưỡng trắng và trị nám theo phương pháp FluorOxygen+C', 90, 1800000),
       (4, 1, 'Collagen Tươi', 'Liệu trình chăm sóc da mặt với collagen tươi', 55, 1250000),
       (5, 1, 'Peeling da', 'Liệu trình peeling da chuyên sâu', 70, 2500000),
       (6, 2, 'Massage body', 'Massage toàn thân thư giãn', 60, 390000),
       (7, 2, 'Thải độc ruột - Massage nội tạng', 'Liệu trình thải độc ruột kết hợp massage nội tạng', 60, 750000),
       (8, 2, 'Massage Nâng cơ vòng 1', 'Liệu trình massage nâng cơ vùng ngực', 60, 950000),
       (9, 2, 'Massage Cổ, Vai, Gáy', 'Massage tập trung vào vùng cổ, vai, gáy', 60, 350000),
       (10, 2, 'Gội đầu chống rụng tóc kết hợp massage cổ, vai, gáy', 'Liệu trình gội đầu và massage', 45, 160000),
       (11, 2, 'Xông hơi ngải cứu', 'Liệu trình xông hơi với ngải cứu', 60, 400000),
       (12, 3, 'Giảm béo bụng', 'Liệu trình giảm béo vùng bụng', NULL, 600000),
       (13, 3, 'Giảm béo eo', 'Liệu trình giảm béo vùng eo', NULL, 400000),
       (14, 3, 'Giảm béo mông', 'Liệu trình giảm béo vùng mông', NULL, 400000),
       (15, 3, 'Giảm béo tay', 'Liệu trình giảm béo vùng tay', NULL, 400000),
       (16, 3, 'Giảm béo chân', 'Liệu trình giảm béo vùng chân', NULL, 600000),
       (17, 3, 'Giảm béo toàn thân', 'Liệu trình giảm béo toàn thân', NULL, 2400000),
       (18, 4, 'Triệt lông Bikini', 'Liệu trình triệt lông vùng bikini', NULL, 800000),
       (19, 4, 'Triệt lông nách', 'Liệu trình triệt lông vùng nách', NULL, 250000),
       (20, 4, 'Triệt lông chân (nửa chân)', 'Liệu trình triệt lông nửa chân', NULL, 350000),
       (21, 4, 'Triệt lông chân (nguyên chân)', 'Liệu trình triệt lông toàn bộ chân', NULL, 700000),
       (22, 4, 'Triệt lông tay', 'Liệu trình triệt lông vùng tay', NULL, 450000),
       (23, 4, 'Triệt lông mặt, ria mép', 'Liệu trình triệt lông vùng mặt và ria mép', NULL, 250000),
       (24, 4, 'Triệt lông toàn thân', 'Liệu trình triệt lông toàn thân', NULL, 2200000),
       (25, 5, 'Xăm mày, môi nghệ thuật', 'Dịch vụ xăm mày và môi nghệ thuật', NULL, 4000000);

-- Update service_combos
UPDATE services
SET combo_5_price = single_price * 4,
...
INSERT INTO addresses (id, user_id, address, address_type)
SELECT 1, id, '123 Đường ABC, Quận 1, TP.HCM', 'home'
FROM users
WHERE email = 'admin@example.com'
UNION ALL
SELECT 2, id, '456 Đường XYZ, Quận 2, TP.HCM', 'work'
FROM users
WHERE email = 'user@example.com';

-- Chèn dữ liệu vào bảng vouchers
INSERT INTO vouchers (id, code, description, discount_type, discount_value, start_date, end_date)
VALUES (1, 'WELCOME10', 'Giảm 10% cho khách hàng mới', 'percentage', 10, '2023-01-01', '2023-12-31'),
       (2, 'SUMMER50K', 'Giảm 50,000đ cho đơn hàng từ 500,000đ', 'fixed_amount', 50000, '2023-06-01', '2023-08-31');
...
INSERT INTO orders (id, user_id, payment_method_id, total_amount, status)
SELECT 1, id, 1, 1500000, 'completed'
FROM users
WHERE email = 'user@example.com';

-- Chèn dữ liệu vào bảng order_items
INSERT INTO order_items (id, order_id, item_type, item_id, quantity, price)
SELECT 1, o.id, 'product', p.id, 1, p.price
FROM orders o
         JOIN users u ON o.user_id = u.id
         JOIN products p ON p.name = 'FAITH Members Club Face Lamela Veil EX Cleansing'
WHERE u.email = 'user@example.com'
UNION ALL
SELECT 2, o.id, 'service', s.id, 1, s.single_price
FROM orders o
         JOIN users u ON o.user_id = u.id
...
INSERT INTO appointments (id, user_id, service_id, staff_user_id, start_time, end_time, appointment_type, status)
SELECT 1,
       u.id,
       s.id,
       staff.id,
...
FROM users u
         ...
         JOIN services s ON s.service_name = 'Chăm da Amino - Phù hợp mọi loại da'
         JOIN orders o ON o.user_id = u.id
         JOIN order_items oi ON oi.order_id = o.id AND oi.item_type = 'service' AND oi.item_id = s.id
WHERE u.email = 'user@example.com'
LIMIT 1;
